...
ami_assert(ver.compare(ELI_LIB_VERSION, "0.36.0-alpha") >= 0,
	"Invalid ELI_LIB_VERSION (" .. tostring(ELI_LIB_VERSION) .. ")!", EXIT_INVALID_ELI_VERSION)
...
function am.execute(cmd, args)
	...
	local result, err, metadata = cli.process(interface, args)

	-- Handle unknown command/option with suggestions
	if not metadata.executed and metadata.invalid_command_or_option_name then
		local error_msg = err .. " - '" .. metadata.invalid_command_or_option_name .. "'"
		...
		local function print_suggestions(suggestions)
			if #suggestions == 1 then
				print("Did you mean: " .. format_suggestion(suggestions[1]) .. "?")
			else
				print"Did you mean:"
		...
		-- Print suggestions only in TTY mode (not in logs)
		local is_tty = require"is_tty".is_stdout_tty()
		if is_tty then
			if err == "unknown command" and metadata.available_commands then
				local suggestions = cli.find_similar(metadata.invalid_command_or_option_name,
					metadata.available_commands)
				if #suggestions > 0 then
					print_suggestions(suggestions)
				end
			elseif err == "unknown option" and metadata.available_options then
				local suggestions = cli.find_similar(metadata.invalid_command_or_option_name,
					metadata.available_options)
				if #suggestions > 0 then
					print_suggestions(suggestions)
				end
			end
		...
		os.exit(EXIT_CLI_CMD_UNKNOWN)
	end

	ami_assert(metadata.executed, err or "unknown", EXIT_CLI_ACTION_EXECUTION_ERROR)
	return result
end
...
function am.configure_cache(cache)
	if type(cache) == "string" then
		...
		if not ok then
			...
			log("access to '" ..
				am.options.CACHE_DIR .. "' denied (error: " .. tostring(err) .. ") - using local '.ami-cache' directory")
